...
let create_sensor = {
  let query =
    R.find(
      T.(tup4(T.string, T.string, T.int, T.int)),
      T.int,
      {|
  ...
  (name, description, key_id, step, module Db: DB) => {
    let%lwt result = Db.find(query, (name, description, key_id, step));
...
let create_user_relationship = {
  ...
  (user_id, sensor_id, module Db: DB) => {
    let%lwt result = Db.exec(query, (user_id, sensor_id));
    Caqti_lwt.or_fail(result);
  };
};

let create = (user_id, name, description, step, db) => {
  let%lwt k = Api_key.create((), db);
  let%lwt id = create_sensor(name, description, k.id, step, db);
  let%lwt _ = create_user_relationship(user_id, id, db);
...
let get = {
  let query =
    R.find_opt(
      T.(tup2(T.int, T.int)),
      T.(tup4(T.string, T.string, T.string, T.int)),
      {|
...
let from_key = {
  let query =
    R.collect(
      T.string,
      T.tup4(T.int, T.string, T.string, T.int),
      {|
